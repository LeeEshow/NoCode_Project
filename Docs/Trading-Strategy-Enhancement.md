...
- ~~尚未支援「使用者已執行某批」的 endpoint。~~ → `POST /tranches/{batch}/executions` 已實作（BE-TSD-01）
- ~~`_tranche_to_dto()` 尚未輸出 `executedAt`、`executedPrice`、`executedShares`。~~ → 已改輸出 `executions[]`
- ~~`_compute_strategy_status()` 尚未處理全部批次完成後的 completed 狀態。~~ → 已加入 `completed` 判斷
- ~~`StrategyStatus` 後端 / 前端型別尚未包含 `completed`。~~ → 已補齊
...
- ~~尚未支援 `executed` 批次狀態。~~ → `TrancheStatus` 已補 `executed`，`executions[]` 支援多筆（FE-TSD-04）
- ~~尚未提供「確認執行」表單與 API 呼叫。~~ → Modal inline 手動標記 + addExecution viewmodel 已完成（FE-TSD-05）
- ~~尚未分析交易策略方向與再平衡方向是否衝突。~~ → `analyzeDirectionConflict()` 已完成（FE-TSD-01）
- ~~`TradingStrategyModal` 已有 `suggestion` prop，但目前只顯示建議，尚未做衝突解讀。~~ → `.tsm-insight` 衝突區塊已完成（FE-TSD-02）
- ~~`HoldingsTable.StrategyBadge` 尚未顯示已完成批次或衝突提示。~~ → 完成狀態 `AI 已完成` 已實作（FE-TSD-06）；衝突提示集中在 Modal 內（FE-TSD-03 調整設計）
...
Phase A  批次執行追蹤                 ✅ 已完成（2026-06-13）
Phase C  風險環境覆寫警示             ✅ 已完成（2026-06-13）
Phase D  策略生命週期與交易紀錄串接    ✅ 已完成（2026-06-13）
```
...
> **✅ 已實作（v1.4）**：`TradingStrategyModal` 新增 `marketStateAuto` prop，計算 `shouldWarnMacro`，於 header 下方插入警示區塊。CSS 新增 `.tsm-macro-warn*`。`StockOverviewPage` 從 `snapshotStore` 取 `marketStateAuto` 並傳入。TypeScript 零錯誤。
...
### 7.3 批次選擇 UI（**已實作，v1.5**）

**實際佈局**：Modal 有批次時從 `sm`（380px）升為 `lg`（720px），左右二欄。
...
│                          │  ● 不連結                        │
│  交易日期 *              │                                   │
│  [2026/06/13]            │  ◉ 第 1 批  50.00–51.50  10%  [▾]│
│                          │    [已觸發]                      │
│  交易總金額（含手續費）* │    ─────────────────────         │
│  [___________]           │    建議數量  500 股              │
│                          │    觸發條件：...                 │
│  股數（股）*             │                                   │
│  [___________]           │  ○ 第 2 批  52.00–54.00  10%  [▾]│
│                          │    [觀察中]                      │
│  備註                    │                                   │
│  [________________]      │                                   │
│                          │                                   │
...
**RadioDot**：以 inline `<span>` 圓點（border-radius 50%，`border / background` 切換 accent 色）實作，無需額外 CSS class。  
**系統預選**：優先 `triggered` 批次並自動展開；無 triggered 時取最小 batch 號的 `pending` 批次。  
**展開/收折**：點選卡片選取批次；點選右側箭頭獨立切換展開（不影響選取）；展開顯示建議數量與觸發條件。  
**使用者可改選**：`不連結` 選項常駐頂部，點選任一批次或「不連結」切換選取。  
**不做累計強制追蹤**：目標達成率為 runtime 計算的參考值，非強制合約。
...
### ✅ BE-TSD-01：execute tranche endpoint（已完成）

- Pydantic `AddExecutionBody`（`executedPrice`、`executedShares`、`transactionId?`、`executedAt?`），驗證失敗回 422。
- `POST /{stock_code}/tranches/{batch}/executions`：append 執行紀錄、tranche.status → `executed`、重算 strategy.status。
- `_tranche_to_dto()` 新增 `executions[]` 輸出；舊版 fallback tranche 補 `"executions": []`。
- `_compute_strategy_status()` 新增 `completed` 判斷：所有 tranche 皆 `executed` 或 `skipped` → `completed`。
- pytest 新增 11 個案例，45/45 全通過。

### ✅ FE-TSD-04：addExecution model / viewmodel（已完成）

- `types/index.ts`：新增 `TrancheExecution` 介面、`StrategyTranche.executions`、`TrancheStatus` 補 `executed`、`StrategyStatus` 補 `completed`。
- model 新增 `addTrancheExecution()`（POST `/executions`）。
- viewmodel 新增 `addExecution()`，樂觀 append 至 `executions[]` 並即時更新 `strategy.status`，API 失敗 rollback。
- `useTradingStrategyViewModel` 回傳 `addExecution`；`StockOverviewPage` 接線。

### ✅ FE-TSD-05：TradingStrategyModal 執行確認 UI（已完成）

- `TrancheRow` 新增 `onAddExecution` prop。
- **按鈕顯示條件（調整）**：`status` 不是 `pending` / `waiting` / `skipped` 時顯示；`triggered` 顯示「手動標記執行」，`executed` 顯示「新增執行」（支援多次追加）。
- Inline 表單：成交均價預填現價（或 priceHigh）、股數預填 AI 建議股數，驗證後呼叫 `onAddExecution`。
- `executed` 狀態顯示 `executions[]` 列表（日期、股數、均價、txnId）+ 累計摘要（totalShares、avgPrice、pctDone）。
- CSS 新增 `.tsm-tranche--executed`、`.tsm-tranche__exec-*`。
- Modal header status badge 補 `completed` → 顯示「已完成」，variant = `flat`。

### ✅ FE-TSD-06：StrategyBadge 完成狀態（已完成）

- `HoldingsTable.StrategyBadge`：`strategy.status === 'completed'` 或 client-side `isCompleted` 判斷（所有 tranche executions.length > 0 或 skipped）→ 顯示 `AI 已完成`，variant = `flat`。
- TypeScript build 通過。

### ✅ FE-TSD-07：交易紀錄表單批次選擇 UI（已完成）

- Modal 有可連結批次時從 `sm` 升為 `lg`，採左右二欄佈局（左欄 260px 表單，右欄 `flex:1` 批次選擇）。
- 右欄頂部顯示「連結 AI 策略批次」標題（`tips_and_updates` icon）。
- 單選選項：「不連結」+ 各批次（`tsm-tranche` 卡片 + inline `RadioDot` 圓點指示器）。
- 批次卡片標頭與 TradingStrategyModal 一致：`tsm-tranche__title-group`（批次號、價格帶、佔比）+ `tsm-tranche__header-right`（`StatusBadge` + 展開/收折箭頭）。
- 展開後顯示建議數量與觸發條件文字。
- 選中態復用 `.tsm-tranche--open`；展開/收折操作不影響選取狀態。
- Modal 開啟時自動預選並展開第一個 `triggered` 批次。
- 送出後呼叫 `addExecution()`，帶入 `transactionId`、`executedAt`（交易日台北時間 00:00）。
- `useTransactionsViewModel.addTx()` 改回傳 `TransactionDTO | null` 供呼叫端取得 `transactionId`。
...
- 同方向或 hold 不顯示衝突。
- ~~HoldingsTable badge 右側出現 warning icon。~~ → 調整設計：衝突資訊集中在 TradingStrategyModal `.tsm-insight` 區塊內，表格維持簡潔（FE-TSD-03）。
- TypeScript 零錯誤。

### ✅ Phase A 驗收（已完成）

- `triggered` / `executed` tranche 可點擊「手動標記執行」/「新增執行」按鈕。
- inline 表單可輸入成交均價與股數，驗證 > 0 後送出。
- 送出後立即在 executions 列表顯示新筆（樂觀更新）。
- 同一批次可多次新增 execution，列表累計顯示。
- API 成功後以後端回傳的 DTO 校正 server timestamp。
- API 失敗時 rollback 至送出前狀態。
- 重新整理頁面後 executions 仍存在（存 Firestore）。
- 全部批次 executions.length > 0 或 skipped 後，策略顯示 `AI 已完成`。
- 後端 pytest 45/45 通過。
- 前端 `npx tsc -p tsconfig.app.json --noEmit` 零錯誤。

### ✅ Phase D 驗收（已完成）

- 建立交易紀錄時，若同股票有 triggered/pending 批次，顯示批次選擇器。
- 批次選擇器樣式與 TradingStrategyModal 中的批次列一致（`.tsm-tranche`）。
- 選中態復用 `.tsm-tranche--open`；可點擊再次取消選取（切換式）。
- 送出後：交易紀錄建立成功，且選擇的批次 executions 新增一筆（帶 transactionId + executedAt）。
- 策略 Modal 中對應批次的 executions 列表即時更新（樂觀更新）。
- 未選批次時，交易紀錄正常建立，策略狀態不變。
- 前端 `npx tsc -p tsconfig.app.json --noEmit` 零錯誤。

### ✅ Phase C 驗收（已完成）

- `marketStateAuto === 'risk-off'` 且策略方向為買入時，Modal header 下方顯示警示橫幅。
- 賣出 / 觀察 / `completed` / `dismissed` / `expired` 策略不顯示。
- 不阻止使用者確認執行（純提示）。
- TypeScript 零錯誤。
...
### ~~10.2 completed 狀態需前後端一致~~（已完成）

後端 `_compute_strategy_status()` 已加入 `completed` 判斷；前端 `StrategyStatus`、`TrancheStatus`、Modal badge、HoldingsTable badge 均已同步更新。
...
5. ✅ 實作 Phase C：Risk-Off 環境警示。
6. ✅ 實作 FE-TSD-07：交易紀錄表單批次選擇 UI（Phase D 主體）。
...
- **Phase A 完整落地**：BE-TSD-01、FE-TSD-04、FE-TSD-05、FE-TSD-06 全部完成，後端 pytest 45/45，前端 TypeScript 零錯誤。
- **按鈕顯示條件放寬**：`手動標記執行` / `新增執行` 按鈕改為只要 `status` 不是 `pending` / `waiting` / `skipped` 即顯示（原設計僅 `triggered`）。`executed` 狀態同樣可追加執行紀錄，符合多筆累積的設計意圖。
- **任務清單更新**：BE-TSD-01、FE-TSD-04/05/06 標記已完成；Phase A 驗收標準補實際驗證結果。
- **10.2 completed 狀態**標記為已完成。
- **建議落地順序** 1–4 標記已完成；下一步為 FE-TSD-07（Phase D）。

相較 v1.3，**v1.4** 主要修正：

- **Phase C 完整落地**：`TradingStrategyModal` 新增 `marketStateAuto` prop；`shouldWarnMacro` 條件（risk-off + 買入方向 + 非終態）；header 下方插入 `.tsm-macro-warn` 警示橫幅；CSS 新增 `.tsm-macro-warn*`；`StockOverviewPage` 從 `snapshotStore` 取 `marketStateAuto` 並傳入。TypeScript 零錯誤。
- 建議落地順序 1–5 全部完成；下一步為 **FE-TSD-07（Phase D）**。

相較 v1.4，**v1.5** 主要修正（程式碼）：

- **Phase D 完整落地（FE-TSD-07）**：
  - `useTransactionsViewModel.addTx()` 改回傳 `TransactionDTO | null`，讓呼叫端取得 `transactionId`。
  - `AddTransactionModal` 新增 `strategies?` / `onAddExecution?` props；`useMemo` 計算 `linkableTranches`（`triggered` / `pending` 且未執行的批次）。
  - 表單底部新增「連結至 AI 策略批次」選配區：批次列表復用 `.tsm-tranche` / `.tsm-tranche--open`；點擊切換式選取；顯示批次號、狀態、佔比、參考價格帶。
  - 送出流程：先建立交易紀錄，取得 `transactionId`；若有選定批次，立即呼叫 `strategyVm.addExecution()` 帶入 `transactionId` 與 `executedAt`（交易日 台北時間 00:00）。
  - `StockOverviewPage` 傳入 `strategies={strategyVm.strategies}` 與 `onAddExecution` 閉包。
  - TypeScript 零錯誤。
- 建議落地順序 1–6 全部完成；Phase A / B / C / D 均已落地。

相較 v1.5，**v1.6** 主要修正（文件校對）：

- **§1.1 後端缺口** 全部劃線標記已解決，補對應任務編號。
- **§1.2 前端缺口** 全部劃線標記已解決，補對應任務編號與調整說明。
- **§3** Phase D 加 ✅。
- **§7.3 批次選擇 UI** 更新為實際實作的二欄式佈局說明，補 RadioDot / 展開 / 預選邏輯描述，移除未採用的 `.tx-tranche-radio` CSS。
- **§8 FE-TSD-07** 加 ✅，描述更新為右側欄設計、RadioDot、展開/收折、自動預選等實際行為。
- **§9 Phase B 驗收** 加 ✅，「warning icon」條目劃線並補說明（改為集中在 Modal 內顯示）。
