...
```
Phase B  方向衝突偵測與 UI 提示        ← 先做，純前端，低風險
Phase A  批次執行追蹤                 ← 第二，需後端 API + DTO + 測試
...
### FE-TSD-01：方向衝突 utility

- 新增 `resolveStrategyDirection`
- 新增 `resolveRebalanceDirection`
- 新增 `analyzeDirectionConflict`
- 補 TypeScript 單元測試，如目前專案沒有前端測試框架，可先以型別檢查 + build 驗收。

### FE-TSD-02：TradingStrategyModal 衝突提示

- Header 新增 warning badge。
- 再平衡區塊下方新增 conflict block。
- CSS 新增 `.tsm-conflict*`。

### FE-TSD-03：HoldingsTable 衝突 icon

- `StrategyBadge` 接收 `suggestion`。
- 衝突時顯示 warning icon。
- `HoldingRow` 傳入 `rebalanceSuggestions?.[stockCode]`。
