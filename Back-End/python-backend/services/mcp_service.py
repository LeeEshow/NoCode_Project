    ...
    # ── M9：Tag 寫入工具 ─────────────────────────────────────────────────────────
    {
        ...
        "inputSchema": {
            ...
            "properties": {
                ...
                "target_weight":       {"type": ["number", "null"],"description": "目標配比（百分比），0–100，null = 不設目標（選填）"},
...
async def _get_portfolio_tag_analysis() -> dict:
    ...
    for sc, hv in holding_values.items():
        for st in stock_tag_map.get(sc, []):
            ...
            contribution = (hv["value"] * weight_ratio) / total_value * 100  # 百分比
    ...
    for tag_name, td in tag_accum.items():
        ...
        target_raw = info.get("targetWeight")
        # Firestore 儲存 0–1，MCP 工具層統一輸出百分比（0–100），方便 AI 閱讀
        target_pct = round(target_raw * 100, 4) if target_raw is not None else None
        actual     = round(td["actualWeight"], 4)   # 百分比（0–100）
        result_tags.append({
            "tagName":      tag_name,
            "targetWeight": target_pct,
            "actualWeight": actual,
            "deviation":    round(actual - target_pct, 4) if target_pct is not None else None,
...
async def _update_tag(arguments: dict) -> dict:
    ...
    if has_target_weight and target_weight is not None:
        if not isinstance(target_weight, (int, float)) or float(target_weight) <= 0 or float(target_weight) > 100:
            return _text({"error": "target_weight must be between 0 and 100 (percentage)"})
        target_weight = round(float(target_weight) / 100, 6)  # 轉為 0–1 存入 Firestore
