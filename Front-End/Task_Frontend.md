...
### 市場指數列
- [x] FIX-01：指數卡片順序固定為「台股大盤 → 台指期 → 景氣燈號 ｜ S&P 500 → 費城半導體 → 其餘國際指數」，不依 API 回傳順序動態排列
- [x] FIX-02：台指期小卡版面改為與台股大盤相同尺寸（114px），標題格式「台指期 盤中」或「台指期 夜盤」，依台灣時間判斷：09:00–13:30 顯示「盤中」；13:30–次日19:00 顯示「夜盤」
- [x] FIX-03：景氣燈號後端 API（`/market/export-indicator`）無資料時，小卡顯示「—」佔位，不隱藏卡片

### 庫存持股
- [x] FIX-04：未實現損益摘要列移至頁面頂部 PanelHeader 橫幅（今日 / 當天成長率 / 未實現損益 / 整體報酬率）
- [x] FIX-05：庫存持股 Panel 補上「新增持股」功能入口（➕ 按鈕）— 可手動新增一筆持股（代號 / 均價 / 股數），以首筆買入交易寫入並重算持倉

### 版面 / 導覽
- [x] FIX-06：SideNav 補上手動收折 / 展開切換按鈕（收折時置中、展開時靠右；icon 切換 ‹ / ›；expanded 狀態提升至 MainLayout）

### 共用元件
- [x] PanelHeader：每頁頂部橫幅（左側彈性 slot + 右側流動資金輸入框），已套用於 StockOverviewPage / AssetsPage / PlanPage / ReportPage / SettingsPage
